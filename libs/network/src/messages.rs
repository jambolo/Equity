...
mod tests {
    ...
    fn test_simple_messages() {
        // verack and getaddr payloads are empty by Bitcoin protocol;
        // the bridge succeeds iff it returns Ok.
        assert!(SimpleMessage::verack().is_ok());
        assert!(SimpleMessage::getaddr().is_ok());
    }
