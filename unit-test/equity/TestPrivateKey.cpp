    ...
    TEST_METHOD(Equity_PrivateKey_constuctor_uint8_t_ptr)
    {
        for (auto const & c : WIF_CASES)
        {
            ...
            if (result.valid())
            {
                ...
                Assert::IsFalse(result.compressed());                                                       // Should not be
                                                                                                            // compressed
                ...
                Assert::IsTrue(result.compressed());                                                        // Should be compressed
                                                                                                            // now
                Assert::AreEqual(c.wifCompressed, result.toWif(BITCOIN_PRIVATE_KEY_VERSION).c_str());       // Compressed WIF should
                                                                                                            // match
