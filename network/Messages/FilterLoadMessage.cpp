    ...
    : Message(TYPE)
{
    ...
    nHashFuncs_ = Endian::little(P2p::deserialize<uint32_t>(in, size));
    if (nHashFuncs_ > MAX_NUM_HASH_FUNCIONS)
        throw InvalidMessageError();
    tweak_ = Endian::little(P2p::deserialize<uint32_t>(in, size));
...
void FilterLoadMessage::serialize(std::vector<uint8_t> & out) const
{
    ...
    P2p::serialize(Endian::little(nHashFuncs_), out);
