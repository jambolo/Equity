#include "include/equity/Address.h"
...
TEST(EquityAddressTest, constuctor_string)
{
...
TEST(EquityAddressTest, constuctor_uint8_t_ptr)
{
    for (auto const & c : CASES)
    {
        ...
        if (result.valid())
        {
            std::array<uint8_t, Address::SIZE> value = result.value();
            EXPECT_TRUE(std::equal(value.begin(), value.end(), c.data));
            EXPECT_EQ(c.base58Check, result.toString(c.version).c_str());
        }
    }
}

TEST(EquityAddressTest, constuctor_Ripemd160Hash)
{
    for (auto const & c : CASES)
    {
...
TEST(EquityAddressTest, constuctor_publicKey)
{
    for (auto const & c : PUBLIC_KEY_CASES)
    {
        ...
        {
            std::array<uint8_t, Address::SIZE> value = result.value();
            EXPECT_EQ(c.address, result.toString(MAIN_NETWORK_ID).c_str());
        }
    }
}
