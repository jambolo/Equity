#include "include/equity/Base58Check.h"
...
TEST(EquityBase58CheckTest, encode_uint8_t_ptr)
{
...
TEST(EquityBase58CheckTest, encode_vector)
{
    for (auto const & c : CASES)
    {
...
TEST(EquityBase58CheckTest, encode_array)
{
    for (auto const & c : CASES)
    {
        if (c.size == 20)
...
TEST(EquityBase58CheckTest, decode_cstring)
{
    ...
    for (auto const & c : CASES)
    {
        ...
        EXPECT_EQ(c.version, resultVersion);
    }
}

TEST(EquityBase58CheckTest, decode_string)
{
    for (auto const & c : CASES)
    {
        std::vector<uint8_t> resultVector;
...
TEST(EquityBase58CheckTest, decode_cstring_buffer)
{
    for (auto const & c : CASES)
    {
