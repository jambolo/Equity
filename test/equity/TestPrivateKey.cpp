...
static WifInput WIF_CASES[] =
{
    ...
    },
    {
        {
            0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFE,
            0xBA, 0xAE, 0xDC, 0xE6, 0xAF, 0x48, 0xA0, 0x3B, 0xBF, 0xD2, 0x5E, 0x8C, 0xD0, 0x36, 0x41, 0x41         // 1 more than
                                                                                                                   // max
        },
...
static WifValidityInput const WIF_VALIDITY_CASES[] =
{
    ...
    { "L5nzLB4U8X8qTSALobGXntqTHfpbZucCb46z6NUChsChwAXRV5JP", false },          // invalid compressed key, flag == 0xff instead of
                                                                                // 0x01
...
TEST(EquityPrivateKeyTest, constuctor_uint8_t_ptr)
{
    for (auto const & c : WIF_CASES)
    {
        PrivateKey result(c.data, sizeof(c.data));

        EXPECT_EQ(c.valid, result.valid());                                                     // Validity should match
        if (result.valid())
        {
            std::array<uint8_t, PrivateKey::SIZE> value = result.value();
            EXPECT_TRUE(std::equal(value.begin(), value.end(), c.data));                        // Value should match
            EXPECT_FALSE(result.compressed());                                                  // Should not be
                                                                                                // compressed
            EXPECT_EQ(Utility::toHex(c.data, sizeof(c.data)), result.toHex());                  // Hex should match
            EXPECT_EQ(c.wif, result.toWif(BITCOIN_PRIVATE_KEY_VERSION).c_str());                // WIF should match
            result.setCompressed(true);
            EXPECT_TRUE(result.compressed());                                                   // Should be compressed
                                                                                                // now
            EXPECT_EQ(c.wifCompressed, result.toWif(BITCOIN_PRIVATE_KEY_VERSION).c_str());      // Compressed WIF should
                                                                                                // match
            ...
            EXPECT_TRUE(result.compressed());                                                   // Should be compressed
                                                                                                // now
            EXPECT_EQ(c.wifCompressed, result.toWif(BITCOIN_PRIVATE_KEY_VERSION).c_str());      // WIF should match
        }
    }
}

TEST(EquityPrivateKeyTest, constuctor_vector)
{
    for (auto const & c : WIF_CASES)
    {
        PrivateKey result(std::vector<uint8_t>(c.data, c.data + sizeof(c.data)));
    ...
    EXPECT_FALSE(result1.valid());                   // Should not be valid

    invalidInput.resize(PrivateKey::SIZE + 1);          // One byte too big
    PrivateKey result2(invalidInput);
    EXPECT_FALSE(result2.valid());                   // Should not be valid
}

TEST(EquityPrivateKeyTest, constuctor_string)
{
    for (auto const & c : WIF_CASES)
    {
        {
            ...
            if (result.valid())
            {
                std::array<uint8_t, PrivateKey::SIZE> value = result.value();
                EXPECT_TRUE(std::equal(value.begin(), value.end(), c.data));             // Value should match
                EXPECT_FALSE(result.compressed());                                       // Should not be compressed
            }
        }
        {
            PrivateKey result(std::string(c.wifCompressed));

            EXPECT_EQ(c.valid, result.valid());
            if (result.valid())
            {
                std::array<uint8_t, PrivateKey::SIZE> value = result.value();
    ...
    {
        PrivateKey result(std::string(c.wif));
        EXPECT_EQ(c.expected, result.valid());
    }
...
TEST(EquityPrivateKeyTest, setCompressed)
{
    GTEST_SKIP();
}

TEST(EquityPrivateKeyTest, value)
{
    GTEST_SKIP();
}

TEST(EquityPrivateKeyTest, compressed)
{
    GTEST_SKIP();
}

TEST(EquityPrivateKeyTest, valid)
{
    GTEST_SKIP();
}

TEST(EquityPrivateKeyTest, toWif)
{
    GTEST_SKIP();
}

TEST(EquityPrivateKeyTest, toHex)
{
    GTEST_SKIP();
}
